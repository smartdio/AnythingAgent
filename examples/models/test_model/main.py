...
class TestModel(AnythingBaseModel):
    """
    Test Model
    This is a simple model used for testing the model deployment mechanism.
    It supports basic message handling and streaming output.
    """
    
    async def on_chat_start(self) -> None:
        """Processing when chat starts"""
        self.clear_context()
    ...
    async def on_chat_messages(
        ...
        In streaming mode, there is a 1-second delay between each output
        """
        ...
        if callback:
            # Streaming mode: send line by line with 1-second delay
